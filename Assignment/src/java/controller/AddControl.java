...
public class AddControl extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String psellID = request.getParameter("sellID");
        
        
        
        DAO dao = new DAO();
        TaiKhoan b = dao.getAccountByuID(psellID);
        if (b == null) {
            request.setAttribute("mess", "Tai Khoan Khong Ton Tai!");
            request.getRequestDispatcher("manager").forward(request, response);
        } else {
            if (b.getIsSell() == 0) {
                request.setAttribute("mess", "Tai Khoan Khong Phai Nguoi Ban Hang!");
                request.getRequestDispatcher("manager").forward(request, response);
            } else {
                dao.insertProduct(pname, pimage, pprice, pquantity, ptitle, pdescription, pcategory, pbrand, psellID, pmodel, pcolor, pdelivery, pimage2, pimage3, pimage4);
